...
{
	return student->ratingPlace1 - student->ratingPlace2;
}
...
void update()
{
	...
	while (current != NULL)
	{
		...
		current->student->ratingDelta = getRatingDelta(current->student);
...
Student* readStudent(FILE* file)
{
	...
	newStudent->lastname = readString(file);
...
void writeStudent(Student* student, FILE* file)
{
	...
	fprintf(file, "%s", student->lastname);
